...
from users.exceptions import STTConnectionError
...
def STT_full_file(in_audio_path, out_audio_path, task_id: int = 1):
    ...
    try:
        final_text = transcribe_long_audio_with_merge(
            audio_file, 3, 2, "fa_IR", task_id
        )
    except Exception as e:
        raise STTConnectionError(
            "در حال حاضر ارتباط با سرویس تبدیل گفتار به متن برقرار نیست."
        ) from e
