...
REQUIRED_COLUMNS = [
    'national_id', 'first_name', 'last_name', 'phone_number',
    'subdepartments_list', 'roles_list', 'password'
]
OPTIONAL_COLUMNS = ['personnel_code', 'subdepartments_list', 'roles_list']
...
def import_employees_from_excel(excel_file):
    ...
    if sheet.max_row < 2:
        return {'success': 0, 'skipped': 0, 'errors': ["فایل اکسل خالی است یا فقط سرستون دارد."]}
    ...
    with transaction.atomic():
        for row_index, row in enumerate(sheet.iter_rows(min_row=2), start=2):
            ...
            if not data['password'] or str(data['password']).strip().lower() in ['none', 'nan', 'null', '']:
                row_errors.append("رمز عبور نمی‌تواند خالی یا نامعتبر باشد.")
                errors.append(f"ردیف {row_index}: رمز عبور نمی‌تواند خالی یا نامعتبر باشد.")
                skipped_count += 1
            ...
            try:
                ...
                if not created:
                    ...
                    if data['personnel_code']:  # فقط اگر مقدار جدید داشته باشه، به‌روزرسانی کن
